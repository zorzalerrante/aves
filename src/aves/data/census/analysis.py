...
def get_urban_zones(zones, remove_artifacts=True, area_threshold=100000):
    ...
    """
    urban_zones = loading.read_limite_urbano_censal()
...
def get_urban_municipalities():
    comunas = loading.read_comuna()
    zones = loading.read_limite_urbano_censal().to_crs(comunas.crs)
    comunas_urbanas = (
...
def overlay_zoning(zoning, crs='EPSG:3857', area_threshold=100000):
    """
    ...
    for r in regiones:
        censo_zoning_by_region.append(loading.read_zona(r))
        censo_zoning_by_region.append(loading.read_localidad(r))
...
def population_by_zoning(intersected_zoning, zoning_unique_id):
    ...
    # get personal_data
    df = loading.read_personas(columnas=['ID_ZONA_LOC', 'PERSONAN'], filtros=[("REGION", "in", region_list)])
...
def sex_by_zoning(intersected_zoning, zoning_unique_id):
    ...
    """
    region_list = intersected_zoning.REGION.unique()
    sex_by_region = loading.read_personas(columnas=['ID_ZONA_LOC', 'P08'], filtros=[("REGION", "in", region_list)])
...
def age_by_zoning(intersected_zoning, zoning_unique_id):
    ...
    region_list = intersected_zoning.REGION.unique()
    age_by_region = loading.read_personas(columnas=['ID_ZONA_LOC', 'P09'], filtros=[("REGION", "in", region_list)])
...
def inmigrants_by_zoning(intersected_zoning, zoning_unique_id):
    ...
    region_list = intersected_zoning.REGION.unique()
    # get personal_data
    inmigrants_by_region = loading.read_personas(columnas=['ID_ZONA_LOC', 'P10','P12'], filtros=[("REGION", "in", region_list)])
...
def schooling_by_zoning(intersected_zoning, zoning_unique_id, min_age=0, bins=5):
    """
    ...
    """
    region_list = intersected_zoning.REGION.unique()
    schooling_by_region = loading.read_personas(columnas=['ID_ZONA_LOC', 'ESCOLARIDAD'], filtros=[("REGION", "in", region_list), ("ESCOLARIDAD", "<", 30), ("P09", ">=", min_age)])
...
def indigenous_by_zoning(intersected_zoning, zoning_unique_id):
    ...
    region_list = intersected_zoning.REGION.unique()
    # get personal_data
    df = loading.read_personas(columnas=['ID_ZONA_LOC', 'P16'], filtros=[("REGION", "in", region_list)])
    df = df[df['P16']=='Sí']
