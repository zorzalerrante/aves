...
class ColoredCurveCollection(object):
    def __init__(self, source_color=None, target_color=None, linewidth=1.0, min_linewidth=None):
    ...
    def render(self, ax, linestyle='solid', alpha=0.75, zorder=0):
        if not self.prepared:
            self.prepare()

        results = []
        for group in self.prepared_data.values():
            collection = LineCollection(group['segments'], cmap=self.colormap, linewidths=group['linewidth'], linestyle=linestyle, alpha=alpha, zorder=zorder)
    ...
    def set_colors(self, source=None, target=None):
        ...
        self.colormap = colors.LinearSegmentedColormap.from_list("", [self.source_color, self.target_color])
    ...
    def prepare(self):
        ...
        for n_points, pairs in self.curves_per_length.items():
            ...
            color_values = np.concatenate(list(repeat(np.linspace(0, 1, num=n_points - 1), len(pairs))))

            if self.min_linewidth is not None:
                linewidth = np.squeeze(self.linewidth_transform(weights.reshape(-1, 1), feature_range=(self.min_linewidth, self.linewidth)))
            else:
