# ported from https://github.com/beyondbeneath/bezier-curved-edges-networkx/
# using the MIT License, which is compatible with GPL3

import numpy as np
import bezier


def curved_edges(edge_list, dist_ratio=0.2, bezier_precision=20, polarity="random"):
    edges = []
    for edge in edge_list:
        edges.append(edge.points)

    edges: np.array = np.array(edges)
    ...
    # )
    coords = np.array(edges).reshape([edges.shape[0], 2, edges.shape[1]])
    # print(coords)
